...
class TelegramService extends ChangeNotifier {
  ...
  Future requestQrCodeAuthentication(List<int> otherUserIds) async {
    final result = await send(
      RequestQrCodeAuthentication(otherUserIds: otherUserIds),
    );
    if (result != null && result is TdError) {
      // Xử lý lỗi
    }
  ...
  Future<int?> searchUserByPhoneNumber(String phoneNumber) async {
    final telegramService = TelegramService();

    final searchQuery = SearchContacts(
      query: phoneNumber,
      limit: 1,
    );
  ...
  Future<List<Message>> getChatHistory(int chatId, int limit) async {
  final telegramService = TelegramService();
